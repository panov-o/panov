class Figure:
    ...
    def set_sides(self, *args):
        ...
        if self._is_valid_sides(self, *args):
            self.get_sides()
        else:
            for i in range(self.sides_count):
                mas_list.append(self.side)
            self.sides = mas_list
            self.get_sides()
...
class Circle(Figure):
    ...
    def set_radius(self):
        self.__radius = self.__len__() / (2 * 3.141592653589793)
...
circle1.set_sides(15) # Изменится (А у меня не изменилось)
